...
+ (id)xyd_doInstanceMethodTarget:(id)target selectorName:(NSString *)selectorName args:(NSArray *)args
{
    ...
    [invocation xyd_setArgumentsWithArray:args];
...
- (void)xyd_setArgumentsWithArray:(NSArray *)array
{
    if (nil != array && array.count > 0) {
        ...
        if (array.count != numargs) {
            ...
            [self xyd_setArgumentWithObject:arg atIndex:0];
        }else{
            for (NSUInteger i = 0; i < numargs; i++) {
                [self xyd_setArgumentWithObject:array[i] atIndex:i];
            }
        }
    }
}
